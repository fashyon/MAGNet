...
from model import Multi_Aggregation_Network as MAGNet
...
class Session:
    def __init__(self):
        ...
        if torch.cuda.is_available():
            self.net = MAGNet().cuda()
        if len(device_ids) > 1:
            self.net = nn.DataParallel(MAGNet()).cuda()
        self.opt_net = Adam(self.net.parameters(), lr=settings.lr)
        self.sche_net = MultiStepLR(self.opt_net, milestones=[(settings.epoch+1)*settings.one_epoch_step], gamma=0.1)
        self.ssim = SSIM().cuda()
        self.dataloaders = {}
